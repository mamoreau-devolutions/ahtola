...
| `compile-attach-cross-database-support` | partial | s2-capability | L | 65 | 8 | Managed ATTACH requires file-backed primary and forbids cross-database statements entirely; temp/main mixed statements are rejected as cross-database. Blocks the entire 4… |
...
| `compile-attach-same-file-not-supported` | missing | s4-intentional | S | 61 | 2 | "Managed ATTACH requires a file-backed managed primary database so attachments share its file system" -- attaching the same in-memory/managed primary database to itself (… |
